...
panelRouter.get("/alquileres-del-dia", async (req, res, next) => {
    try {
        const data = await getBookingsToday(req.query.fecha);
...
panelRouter.get("/ingreso-del-dia", async (req, res, next) => {
    try {
        const data = await getIncomeToday(req.query.fecha);
        res.status(200).json(data);
    }
    catch (err) {
        next(err);
...
panelRouter.get("/pendientes-del-dia", async (req, res, next) => {
    try {
        const data = await getPendingPaymentsToday(req.query.fecha);
        res.status(200).json(data);
    }
    catch (err) {
        next(err);
