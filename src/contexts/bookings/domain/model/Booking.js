...
export function overlaps(candidate, existing) {
    return candidate.startTime < existing.endTime && candidate.endTime > existing.startTime;
}
export function hasConflict(candidate, activeBookings, excludeBookingId) {
...
export function resolvePaymentStatus(totalAmount, paidAmount) {
    if (paidAmount <= 0)
        return "PENDING";
...
export function assertWithinOperatingHours(openTime, closeTime, range) {
...
function getPeruDateTimeParts(now) {
...
// fija.
export function assertNotInPast(date, startTime, now = new Date()) {
