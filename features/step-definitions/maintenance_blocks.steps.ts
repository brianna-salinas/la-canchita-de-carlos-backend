...
When("the administrator blocks {string} on {string} from {string} to {string} with reason {string}", async function (
  this: CustomWorld,
  court: string,
  date: string,
  startTime: string,
  ...
  (this as any)._lastBlockContext = { court, date, startTime, endTime };
});

Given("{string} has a maintenance block on {string} at {string}", async function (this: CustomWorld, court: string, date: string, time: string) {
...
Given("{string} has blocks registered on {string}", async function (this: CustomWorld, court: string, date: string) {
  await this.scheduleBlocks.createMany(
    courtId(this, court),
    new Date(date),
    [new Date("1970-01-01T09:00:00Z"), new Date("1970-01-01T10:00:00Z")],
    "Lawn maintenance"
  );
});

Given("a block with id {int} exists for {string}", async function (this: CustomWorld, id: number, court: string) {
...
When("the administrator requests the blocks for {string} on {string}", async function (this: CustomWorld, court: string, date: string) {
...
When("the administrator deletes block {int}", async function (this: CustomWorld, id: number) {
  const unblockSchedule = makeUnblockSchedule({ scheduleBlocks: this.scheduleBlocks });
  await unblockSchedule(id);
});

Then("a block is created for each hour within that range", async function (this: CustomWorld) {
  const ctx = (this as any)._lastBlockContext;
  const expectedCount = hoursBetween(ctx.startTime, ctx.endTime);
  assert.equal((this.lastResult as any[]).length, expectedCount);
});

Then("those hours no longer appear as available for booking", async function (this: CustomWorld) {
  const ctx = (this as any)._lastBlockContext;
...
Then("they receive the list of blocked hours with their reasons", async function (this: CustomWorld) {
  const blocks = this.lastResult as any[];
  assert.ok(blocks.length > 0);
  for (const b of blocks) assert.ok(b.reason, "Expected every block to carry its reason.");
});

Then("that hour becomes available for booking again", async function (this: CustomWorld) {
