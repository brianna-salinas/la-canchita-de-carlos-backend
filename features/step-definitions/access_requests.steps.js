...
Given("a pending request from {string} exists", async function (name) {
    const email = `${name.toLowerCase().replace(/\s+/g, ".")}@example.com`;
    const request = await this.accessRequests.create({ name, email, passwordHash: "fake-hash" });
    this.lastRequestId = request.id;
});
Given("a request from {string} that was already approved exists", async function (name) {
    const email = `${name.toLowerCase().replace(/\s+/g, ".")}@example.com`;
    const request = await this.accessRequests.create({ name, email, passwordHash: "fake-hash" });
...
When("a person submits an access request with name {string} and email {string}", async function (name, email) {
    ...
    this.lastRequestId = this.lastResult.id;
});
When("the owner approves that request", async function () {
...
When("the owner rejects that request", async function () {
...
When("the owner tries to approve it again", async function () {
    const authorizeAdminRequest = makeAuthorizeAdminRequest({
        users: this.users,
        accessRequests: this.accessRequests,
        emailVerificationTokens: this.emailVerificationTokens,
...
Then("the request is saved with status {string}", async function (status) {
    const request = await this.accessRequests.findByIdOrThrow(this.lastRequestId);
    assert.equal(request.status, status);
});
Then("every active owner receives an email about the new request", async function () {
    assert.ok(this.notifier.sent.some((s) => s.type === "newAccessRequestAlert"));
});
Then("a user is created with status {string}", async function (status) {
    assert.equal(this.lastResult.status, status);
});
Then("the requester receives an email with the account verification link", async function () {
    assert.ok(this.notifier.sent.some((s) => s.type === "emailVerification"));
});
Then("the requester receives an email saying their request was approved", async function () {
    assert.ok(this.notifier.sent.some((s) => s.type === "adminDecision" && s.data.approved === true));
});
Then("the request status becomes {string}", async function (status) {
    const request = await this.accessRequests.findByIdOrThrow(this.lastRequestId);
    assert.equal(request.status, status);
});
Then("the requester receives an email saying their request was rejected", async function () {
    assert.ok(this.notifier.sent.some((s) => s.type === "adminDecision" && s.data.approved === false));
});
Then("no user is created", async function () {
...
Then("the system responds with the error {string}", async function (_message) {
...
Given("a user exists with status {string} and a valid verification token", async function (status) {
    const user = await this.users.create({
        name: "Test User",
        username: "testuser",
...
Given("a user exists with status {string}", async function (status) {
...
When("the user opens the verification link with that token", async function () {
...
When("that user tries to log in with correct credentials", async function () {
...
Then("the user's account status becomes {string}", async function (status) {
    const userId = this.userIdByName.get("Test User");
    const user = await this.users.findByIdOrThrow(userId);
    assert.equal(user.status, status);
});
Then("the system responds {string}", async function (_message) {
    assert.ok(this.lastError, "Expected an error to have been thrown, but none was.");
    assert.equal(this.lastError.status, 401);
});
// --- Bootstrap del primer owner y ascenso de administradores (TS-bootstrap) ---
Given("no user with isOwner set to true exists yet", async function () {
    // No-op: el repositorio fake empieza vacio, no hace falta sembrar nada.
});
When("someone calls the bootstrap endpoint with the correct setup token", async function () {
...
Then("an active user is created with isOwner set to true", async function () {
    const created = this.lastResult;
    assert.equal(created.isOwner, true);
    const user = await this.users.findByIdOrThrow(created.id);
    assert.equal(user.status, "ACTIVE");
});
Then("a second call to the bootstrap endpoint fails because an owner already exists", async function () {
    const bootstrapOwner = makeBootstrapOwner({ users: this.users });
    try {
...
Given("{string} is an active administrator and is not an owner", async function (name) {
    const user = await this.users.create({
        name,
        username: name.toLowerCase(),
...
When("an owner promotes {string} to owner", async function (name) {
    const promoteToOwner = makePromoteToOwner({ users: this.users });
    this.lastResult = await promoteToOwner(this.userIdByName.get(name));
});
Then("{string} now has isOwner set to true", async function (name) {
    const userId = this.userIdByName.get(name);
    const user = await this.users.findByIdOrThrow(userId);
    assert.equal(user.isOwner, true);
    assert.equal(this.lastResult.id, userId);
});
//# sourceMappingURL=access_requests.steps.js.map